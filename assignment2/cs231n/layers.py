...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == "train":
        ...
        x_hat =(x - Ex) / np.sqrt(var_x + eps) 
        out = x_hat * gamma + beta

        cache = (x,Ex,var_x,x_hat,eps,gamma,beta)
...
def batchnorm_backward(dout, cache):
    ...
    # pass
    # 准备参数
    x, Ex, var_x, x_hat, eps, gamma, beta = cache
    N = x.shape[0]
    mu1 = x - Ex
    mu2 = np.sqrt(var_x + eps)
    ...
    dvar = 0.5 * (var_x + eps) ** (-0.5) * dmu2                   # (D,)
    dx_square = 1.0 / N * dvar
    dmu3 = 2.0 * (x - Ex) * dx_square
    dx2 = dmu3
...
def batchnorm_backward_alt(dout, cache):
    ...
    dx = (1./N) * gamma * (var_x + eps) ** (-1. / 2.) * (
      N * dout - np.sum(dout, axis=0) - 
      (x - Ex) * (var_x + eps) ** (-1.0) * np.sum(dout * (x - Ex),axis=0)
      )
...
def layernorm_backward(dout, cache):
    ...
    dx_hat = gamma * dout
    dvar = np.sum((x - Ex) * dx_hat, axis=1, keepdims=True) * (
                -0.5 / np.sqrt(var_x + eps) ** 3)

    dmean = -1 / np.sqrt(var_x + eps) * np.sum(dx_hat, axis=1, keepdims=True) + dvar * np.sum(
        -2 * (x - Ex), axis=1, keepdims=True) / N

    # final gradient
    dx = 1 / np.sqrt(var_x + eps) * dx_hat + 1 / N * dmean + dvar * 2 * (x - Ex) / N
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    # pass
    stride = conv_param['stride'] # 步幅
    pad = conv_param['pad'] # 补充0
...
def conv_backward_naive(dout, cache):
    ...
    for i in range(H_out):
      for j in range(W_out):
        ...
        for l in range(F):
          dw[l,:,:,:] += np.sum(x_mask * (dout[:,l,i,j])[:,None,None,None],axis=0)
        for n in range(N):
...
def max_pool_forward_naive(x, pool_param):
    ...
    for i in range(H_out):
      for j in range(W_out):
        x_mask = x[:,:,i*stride:i*stride+height,j*stride:j*stride+width]
...
def max_pool_backward_naive(dout, cache):
    ...
    H_out = int(1 + (H - height)/stride)
    W_out = int(1 + (H - width)/stride)

    for i in range(H_out):
      for j in range(W_out):
        x_mask = x[:,:,i*stride:i*stride+height,j*stride:j*stride+width]
        max_mask = np.max(x_mask,axis=(2,3))
        temp_mask = (x_mask == (max_mask)[:,:,None,None])
        dx[:,:,i*stride:i*stride+height,j*stride:j*stride+width] += temp_mask * (dout[:,:,i,j])[:,:,None,None]
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    # after transpose,(N,C,H,W) become (N,H,W,C)
...
def spatial_groupnorm_forward(x, gamma, beta, G, gn_param):
    ...
    Ex = np.mean(x_temp,axis=(2,3,4),keepdims=True)
    var = np.var(x_temp,axis=(2,3,4),keepdims=True)
    x_hat = (x_temp - Ex) / np.sqrt(var + eps)
    x_hat = x_hat.reshape(N,C,H,W)
    out = gamma * x_hat + beta
    cache = (x,gamma,Ex,var,eps,x_hat,G)
...
def spatial_groupnorm_backward(dout, cache):
    ...
    # pass
    x, gamma, Ex, var, eps, x_hat, G = cache
    ...
    dvar = np.sum(dx_hat * (x_temp - Ex) * (-0.5) * np.power((var+eps),-1.5),axis=(2,3,4),keepdims=True)
    dEx = np.sum(dx_hat * (-1)/np.sqrt(var + eps),axis=(2,3,4),keepdims=True) + dvar * np.sum(-2 * (x_temp - Ex),axis=(2,3,4),keepdims=True) / m
    dx = dx_hat / np.sqrt(var + eps) + dvar * 2 * (x_temp - Ex) /m + dEx / m
