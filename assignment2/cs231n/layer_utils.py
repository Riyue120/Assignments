...
def batchnorm_relu_forward(x, gamma, beta, bn_param):
    ...
    a, fc_cache = batchnorm_forward(x, gamma, beta, bn_param)
    out, relu_cache = relu_forward(a)
    cache = (fc_cache, relu_cache)
    return out, cache
...
def batchnorm_relu_backward(dout, cache):
    ...
    fc_cache, relu_cache = cache
    da = relu_backward(dout, relu_cache)
    dx, dw, db = batchnorm_backward_alt(da, fc_cache)
    return dx, dw, db

def layernorm_relu_forward(x, gamma, beta, ln_param):
    a, fc_cache = layernorm_forward(x, gamma, beta, ln_param)
    out, relu_cache = relu_forward(a)
    cache = (fc_cache, relu_cache)
    return out, cache
...
def layernorm_relu_backward(dout, cache):
    fc_cache, relu_cache = cache
    da = relu_backward(dout, relu_cache)
    dx, dw, db = layernorm_backward(da, fc_cache)
    return dx, dw, db
